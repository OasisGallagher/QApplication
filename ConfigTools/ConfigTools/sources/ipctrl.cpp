...
void IPCtrl::setText(const QString& ip) {
	if (ip.isEmpty()) {
		clear(); 
		return;
	}

	QStringList fields = ip.split('.');
	Q_ASSERT(fields.size() == QTUTL_IP_SIZE);

	for (int i = 0; i < QTUTL_IP_SIZE; ++i) {
		m_pLineEdit[i]->setText(fields[i]);
	}
}

void IPCtrl::slotTextChanged(QLineEdit* pEdit) {
	for (unsigned int i = 0; i != QTUTL_IP_SIZE; ++i) {
		if (pEdit != m_pLineEdit[i]) { continue; }
