...
/**
* @file  smart_ptr.hpp
* @brief smart_ptr is a minimal implementation of smart pointer, a subset of the C++11 std::shared_ptr or boost::shared_ptr.
*
...
public:
	...
	smart_ptr(void) throw() : // never throws
		px(NULL),
		pn() {
	}
	/// @brief Constructor with the provided pointer to manage
	explicit smart_ptr(T* p) : // may throw std::bad_alloc
	...
	template <class U>
	smart_ptr(const smart_ptr<U>& ptr, T* p) :
		//px(p), would be unsafe as acquire() may throw, which would call release() in destructor
		pn(ptr.pn) {
		acquire(p);   // may throw std::bad_alloc
	}
	...
	template <class U>
	smart_ptr(const smart_ptr<U>& ptr) throw() : // never throws (see comment below)
												   //px(ptr.px),
		pn(ptr.pn) {
		SHARED_ASSERT((NULL == ptr.px) || (0 != ptr.pn.use_count())); // must be coherent : no allocation allowed in this path
		acquire(static_cast<typename smart_ptr<T>::element_type*>(ptr.px));   // will never throw std::bad_alloc
	}
	/// @brief Copy constructor (used by the copy-and-swap idiom)
	smart_ptr(const smart_ptr& ptr) throw() : // never throws (see comment below)
												//px(ptr.px),
		pn(ptr.pn) {
	...
	smart_ptr& operator=(smart_ptr ptr) throw() // never throws
	{
		swap(ptr);
		return *this;
	}
	/// @brief the destructor releases its ownership
	inline ~smart_ptr(void) throw() // never throws
	{
		release();
	}
	...
	/// @brief Swap method for the copy-and-swap idiom (copy constructor and swap method)
	void swap(smart_ptr& lhs) throw() // never throws
	{
		std::swap(px, lhs.px);
		pn.swap(lhs.pn);
	}
...
private:
	// This allow pointer_cast functions to share the reference counter between different smart_ptr types
	template<class U>
...
template<class T, class U> inline bool operator==(const smart_ptr<T>& l, const smart_ptr<U>& r) throw() // never throws
{
	return (l.get() == r.get());
}
template<class T, class U> inline bool operator!=(const smart_ptr<T>& l, const smart_ptr<U>& r) throw() // never throws
...
smart_ptr<T> static_pointer_cast(const smart_ptr<U>& ptr) // never throws
{
